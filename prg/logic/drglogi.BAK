...
SELECT csp
set order to code
COPY TO ('../../text/'+p_kieli+'/csp') DELIMITED WITH CHAR ';' FIELDS code, text, ncsp, released, usedate
SELECT dg
SET ORDER TO code
COPY TO ('..\..\text\'+language.lan+'\drgdg.txt') DELIMITED WITH CHAR ';'
SELECT icd_10
set order to code
COPY TO ('..\..\text\'+p_kieli+'\icd_10.txt') DELIMITED WITH CHAR ';' FIELDS code, d_code, ast, code_w, d_code_w, text, valid
SELECT dgkat
SET ORDER TO dgcat 
