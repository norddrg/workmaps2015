...
procedure CCSIIR
	...
	do WHILE NOT EOF()
	    ...
	    IF FOUND() AND SUBSTR(kompkat.compl,3,1)<>'I'
	...
	COPY to (lc_siirto+'\kompkat.txt') DELIMITED WITH CHAR ';' 
	LIST STRUCTURE TO (lc_siirto+'\kompkat_str.txt')
	goto Top
	skip
	do WHILE NOT EOF()
		SELECT old
	...
	COPY to (lc_siirto+'\muutos\kompkat.txt')DELIMITED WITH CHAR ';'
	LIST STRUCTURE TO (lc_siirto+'\muutos\kompkat_str.txt')
	USE
	SELECT old
	USE
	return
